import os
import sys
from scripts.isolateShapesInPNG import isolateShapesPNG
from scripts.isolateShapesInSVG import isolateShapesSVG
...
def extract( type ):
    ...
    for filename in os.listdir(sheetDirectory):
        ...
        if os.path.isfile(os.path.join(sheetDirectory, filename)):
            ...
            if ( type == "png"):
              isolateShapesInPNG(sheetDirectory, shapesDirectory, filename)
            else:
              isolateShapesInSVG(sheetDirectory, shapesDirectory, filename)
...
def generate( type ):
    ...
    if ( type == "png"):
      generateDataFromPNG(categories, generated_count, generated_width, generated_height)
    else:
      generateDataFromSVG(categories, generated_count, generated_width, generated_height)
